...
def _get_log_energy(strided_input: Tensor,
    ...
    if energy_floor != 0.0:
        log_energy = torch.max(log_energy, torch.tensor(math.log(energy_floor)).to(strided_input.device))
...
def _get_window(waveform: Tensor,
    ...
    if dither != 0.0:
        ...
        x = torch.max(EPSILON, torch.rand(strided_input.shape)).to(device)
    ...
    if raw_energy:
        ...
        # window function
        signal_log_energy = _get_log_energy(strided_input, EPSILON.to(device), energy_floor)  # size (m)
    ...
    window_function = _feature_window_function(
        window_type, window_size, blackman_coeff, device).unsqueeze(0)  # size (1, window_size)
...
def spectrogram(waveform: Tensor,
    ...
    strided_input, signal_log_energy = _get_window(
        ...
        snip_edges, raw_energy, energy_floor, dither, remove_dc_offset, preemphasis_coefficient, device)
...
def vtln_warp_freq(vtln_low_cutoff: float,
    ...
    res = torch.empty_like(freq).to(device)
...
def vtln_warp_mel_freq(vtln_low_cutoff: float,
    ...
    return mel_scale(vtln_warp_freq(vtln_low_cutoff, vtln_high_cutoff, low_freq, high_freq,
                                    vtln_warp_factor, inverse_mel_scale(mel_freq), device))
...
def get_mel_banks(num_bins: int,
    ...
    return bins.to(device), center_freqs.to(device)
...
def fbank(waveform: Tensor,
    ...
    mel_energies, _ = get_mel_banks(num_mel_bins, padded_window_size, sample_frequency,
                                    low_freq, high_freq, vtln_low, vtln_high, vtln_warp, device)
...
def _get_dct_matrix(num_ceps: int, num_mel_bins: int, device: str) -> Tensor:
    ...
    return dct_matrix.to(device)


def _get_lifter_coeffs(num_ceps: int, cepstral_lifter: float, device: str) -> Tensor:
    # returns size (num_ceps)
    # Compute liftering coefficients (scaling on cepstral coeffs)
    # coeffs are numbered slightly differently from HTK: the zeroth index is C0, which is not affected.
    lifter_coeffs = 1.0 + 0.5 * cepstral_lifter * torch.sin(math.pi * torch.arange(num_ceps) / cepstral_lifter)
    return lifter_coeffs.to(device)
...
def mfcc(
    ...
    feature = fbank(waveform=waveform, blackman_coeff=blackman_coeff, channel=channel,
                    ...
                    window_type=window_type, device=device)
    ...
    dct_matrix = _get_dct_matrix(num_ceps, num_mel_bins, device)
    ...
    if cepstral_lifter != 0.0:
        # size (1, num_ceps)
        lifter_coeffs = _get_lifter_coeffs(num_ceps, cepstral_lifter, device).unsqueeze(0)
        feature *= lifter_coeffs

    # if use_energy then replace the last column for htk_compat == true else first column
    if use_energy:
        feature[:, 0] = signal_log_energy + 21.0
...
def _get_LR_indices_and_weights(orig_freq: float,
    ...
    return min_input_index.to(device), weights.to(device)
