...
class GatedAutoEncoderSum(nn.Module):
	...
	def __init__(self, parameters_encoder, parameters_decoder, parameters_conv, frequency_domain=False, parameters_freq={}, **kwargs):
		...
		super(GatedAutoEncoderSum, self).__init__()
		...
		if self.frequency_domain:
			...
			#self.stft	     = stft(**parameters_stft)
			self.mdct        = mdct()
	...
	def forward(self, input):
		...
		if self.frequency_domain:
			#input, x_phase = self.stft.transform(input.squeeze(dim=2))
			input_MDCT = self.mdct.transform(input.squeeze(dim=2))
			input_MDCT_input = self.mdct.inverse(input_MDCT)

		input_MDCT_normalised = (input_MDCT-input_MDCT.mean())/input_MDCT.std()#(input_MDCT.abs().max())
		...
		#X = self.conv(torch.cat([input, X + P], dim=1))
		...
		X_deNormalised= X*input_MDCT.std()+input_MDCT.mean()#*input_MDCT.abs().max()


		if self.frequency_domain:
			#X = self.stft.inverse(X, x_phase)
